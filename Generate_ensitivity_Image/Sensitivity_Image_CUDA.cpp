...
// load up table; index is module pair; value is corresponding dea number
...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		/*
		int sn1 = 0; // Starting crystal number of volume 1
		int sn2 = NUM_SCANNER_CRYSTALS; // Starting crystal number of volume 2
		int fn1 = NUM_SCANNER_CRYSTALS;
		...
		*/
		...
		if (time_stop_ss > 0)
		{
			...
			int modulePairStart = 0;
			int modulePairEnd = 133;
			PET_data ss_events;
			ss_events.Setup(config_filename, PROMPT);
			float iniBedPosition = ss_events.getInitialBedPosition();
			float endBedPosition = ss_events.getEndBedPosition();
			float imageBedPosition = ss_events.getImageBedPosition();
			float distanceTotal = abs(iniBedPosition - endBedPosition);
			float bedMovementPerStep = ss_events.getBedMOvementStepSize();
			int n_bed_positions = round(abs(iniBedPosition - endBedPosition) / bedMovementPerStep)+1;
			ss_events.CreateFullCoincidenceData(geometry, SS, modulePairStart, modulePairEnd, time);
			if (ss_events._protocol != STATIC)
			{	cout<<"calculating sensitivity for CBM/Step and shoot"<<endl;
				cout << "initial bed position: " << iniBedPosition << endl;
				cout << "end bed position: " << endBedPosition << endl;
				cout << "reconstructed image is at bed position: " << imageBedPosition << endl;
				cout << "bed step size: " << bedMovementPerStep << endl;
				cout << "number of bed posistion" << n_bed_positions << endl;
				...
				for (int n_bed = 0; n_bed < n_bed_positions; n_bed++)
				{
					if (n_bed == 0)
					{
						cout << "bed posistion 1, shifting scanner to initial bed position ..." << endl;
						geometry.shift_all_crystal(iniBedPosition - imageBedPosition);
					}
					else{
						cout<<"bed position "<<n_bed<<endl;
						geometry.shift_all_crystal(bedMovementPerStep);
					}
					recon.Setup_image();
					int GPU_step = 5;
					for (int n = 1; n <= GPU_step; n++)
					{
						...
						recon.Backward_Projection_Attenuation(geometry, movement, sub_ss_events, atten_image, sub_sensitivity_image);
						sensitivity_image.AddFromImageArray(sub_sensitivity_image);
						sub_ss_events.PET_LST_event_list.clear();
					}

					sstm.str("");
					sstm << img_path << output_image_filename_prefix << "_SS_bed_" << n_bed << "_sensitivity.img";
					sensitivity_image.WriteToFile(sstm.str());
					recon.Release_image();
					sensitivity_image.SetValue(0.0f);

				}
				geometry.shift_all_crystal(-n_bed_positions * bedMovementPerStep);
				ss_events.PET_LST_event_list.clear();
			}
			else
			{	
				cout <<"calculating sensitivity image for STATIC" <<endl;
				cuda_em_recon recon(global_parameters, output_image_filename_prefix, img_path);
				recon.Setup_image();
				int GPU_step = 10;
				for (int n = 1; n <= GPU_step; n++)
				...
				sstm.str("");
				sstm << img_path << output_image_filename_prefix << "_SS_sensitivity.img";
				sensitivity_image.WriteToFile(sstm.str());
				recon.Release_image();
				sensitivity_image.SetValue(0.0f);
			}
		...
		if (time_stop_is > 0)
		{
			cout << "\n Generating Sensitivity Image for OS data without normalization...\n";
			int modulePairStart = 134;
			int modulePairEnd = 155; // include the ending index
			PET_data os_events;
			os_events.Setup(config_filename, PROMPT);
			float iniBedPosition = os_events.getInitialBedPosition();
			float endBedPosition = os_events.getEndBedPosition();
			float imageBedPosition = os_events.getImageBedPosition();
			float distanceTotal = abs(iniBedPosition - endBedPosition);
			float bedMovementPerStep = os_events.getBedMOvementStepSize();
			int n_bed_positions = round(abs(iniBedPosition - endBedPosition) / bedMovementPerStep)+1;
			os_events.CreateFullCoincidenceData(geometry, IS, modulePairStart, modulePairEnd, time);
			if (os_events._protocol != STATIC)
			{	cout<<"calculating sensitivity for CBM/Step and shoot"<<endl;
				cout << "initial bed position: " << iniBedPosition << endl;
				cout << "end bed position: " << endBedPosition << endl;
				cout << "reconstructed image is at bed position: " << imageBedPosition << endl;
				...
				for (int n_bed = 0; n_bed < n_bed_positions; n_bed++)
				{
					...
					for (int n = 1; n <= GPU_step; n++)
					{
						PET_data sub_os_events(os_events, GPU_step, n);
						recon.Backward_Projection_Attenuation(geometry, movement, sub_os_events, atten_image, sub_sensitivity_image);
						sensitivity_image.AddFromImageArray(sub_sensitivity_image);
						sub_os_events.PET_LST_event_list.clear();
					}

					sstm.str("");
					sstm << img_path << output_image_filename_prefix << "_OS_bed_" << n_bed << "_sensitivity.img";
					sensitivity_image.WriteToFile(sstm.str());
					recon.Release_image();
					sensitivity_image.SetValue(0.0f);
				}
				geometry.shift_all_crystal(-n_bed_positions * bedMovementPerStep);
				os_events.PET_LST_event_list.clear();
			}
			else
			{	
				cout <<"calculating sensitivity image for STATIC" <<endl;
				...
				sstm.str("");
				sstm << img_path << output_image_filename_prefix << "_OS_sensitivity.img";
				sensitivity_image.WriteToFile(sstm.str());
				recon.Release_image();
				sensitivity_image.SetValue(0.0f);
			}
		...
		if (time_stop_ii > 0)
		{
			cout << "\n Generating Sensitivity Image for OO data without normalization...\n";
			int modulePairStart = 155;
			int modulePairEnd = 155; // include the ending index
			PET_data oo_events;
			oo_events.Setup(config_filename, PROMPT);
			float iniBedPosition = oo_events.getInitialBedPosition();
			float endBedPosition = oo_events.getEndBedPosition();
			float imageBedPosition = oo_events.getImageBedPosition();
			float distanceTotal = abs(iniBedPosition - endBedPosition);
			float bedMovementPerStep = oo_events.getBedMOvementStepSize();
			int n_bed_positions = round(abs(iniBedPosition - endBedPosition) / bedMovementPerStep)+1;
			oo_events.CreateFullCoincidenceData(geometry, II, modulePairStart, modulePairEnd, time);
			if (oo_events._protocol != STATIC)
			{	cout<<"calculating sensitivity for CBM/Step and shoot"<<endl;
				cout << "initial bed position: " << iniBedPosition << endl;
				cout << "end bed position: " << endBedPosition << endl;
				cout << "reconstructed image is at bed position: " << imageBedPosition << endl;
				...
				for (int n_bed = 0; n_bed < n_bed_positions; n_bed++)
				{
					...
					for (int n = 1; n <= GPU_step; n++)
					{
						PET_data sub_oo_events(oo_events, GPU_step, n);
						recon.Backward_Projection_Attenuation(geometry, movement, sub_oo_events, atten_image, sub_sensitivity_image);
						sensitivity_image.AddFromImageArray(sub_sensitivity_image);
						sub_oo_events.PET_LST_event_list.clear();
					}

					sstm.str("");
					sstm << img_path << output_image_filename_prefix << "_OO_bed_" << n_bed << "_sensitivity.img";
					sensitivity_image.WriteToFile(sstm.str());
					recon.Release_image();
					sensitivity_image.SetValue(0.0f);
				}
				geometry.shift_all_crystal(-n_bed_positions * bedMovementPerStep);
				oo_events.PET_LST_event_list.clear();
			}
			else
			{	
				cout <<"calculating sensitivity image for STATIC" <<endl;
				...
				sstm.str("");
				sstm << img_path << output_image_filename_prefix << "_OO_sensitivity.img";
				sensitivity_image.WriteToFile(sstm.str());
				recon.Release_image();
				sensitivity_image.SetValue(0.0f);
			}
