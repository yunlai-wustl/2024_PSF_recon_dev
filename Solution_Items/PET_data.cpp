...
PET_data::PET_data(PET_data &data_from, float time_start, float time_end, PET_coincidence_type t, int time_gap)
{
	int i;
	float time;
	if (data_from.scatter_data.size() > 0)
		scatter_data.reserve(data_from.scatter_data.size());
	...
	{
		// if(data_from.PET_LST_event_list.at(i).src_id>=NUM_SCANNER_CRYSTALS+NUM_INSERT_CRYSTALS ||data_from.PET_LST_event_list.at(i).dest_id >=NUM_SCANNER_CRYSTALS+NUM_INSERT_CRYSTALS )
		//	continue;
		...
		{
			if (data_from.scatter_data.size() > 0)
				scatter_data.push_back(data_from.scatter_data[i]);
...
PET_data::PET_data(PET_data &data_from, int num_subsets, int subset)
{
	...
	//_source = data_from._source;
	//_data_type = data_from._data_type;
	_num_event = (data_from.PET_LST_event_list.size()) / num_subsets;
	//_source = data_from._source;
...
PET_data::~PET_data()
{
	PET_LST_event_list.clear();
	PET_LST_events_pos_list_1.clear();
	scatter_data.clear();
	printf("clearing PET data\n");
}

const unsigned char
	PET_data::graycode[11] = {0x00, 0x10, 0x30, 0x20, 0x60, 0x70, 0x50, 0x40, 0x00, 0x10, 0x30};

void PET_data::add_data(PET_data &data_from, float time_start, float time_end, PET_coincidence_type t)
{
	int i;
	float time;
...
int PET_data::GetDataListLength()
{
	return (int)PET_LST_event_list.size();
}

void PET_data::Setup(const std::string config_file_name, PET_data_type type)
{
	...
	config.GetValue<float>("End bed position", _end_bed_position);
	config.GetValue<float>("Image bed position", _image_bed_position);
	config.GetValue<float>("bed movement step size", _bed_movement_step_size);
	config.GetValue<float>("Max ring difference allowed(mm)", _MAX_Z_ALLOWED);
	...
	{
		_protocol = STATIC;
		cout << "static scan" << endl;
	}
	if (_scan_protocol == "CBM")
	{
		_protocol = CBM;
		cout << "CBM scan"
			 << ", initial bed position = " << _initial_bed_position
			 << ", end bed position = " << _end_bed_position
			 << ", reconstructed image bed position: " << _image_bed_position
			 << ", sensitivity calculation using step size" << _bed_movement_step_size << endl;
	}
	if (_scan_protocol == "Step and Shoot")
	{
		_protocol = STEP_AND_SHOOT;
		cout << "Step and shoot scan, initial bed position = " << _initial_bed_position
			 << ", initial bed position = " << _initial_bed_position
			 << ", end bed position = " << _end_bed_position
			 << ", reconstructed image bed position: " << _image_bed_position
			 << ", sensitivity calculation using step size" << _bed_movement_step_size << endl;
...
void PET_data::ReadFromFile(const std::string filename, PET_geometry geometry)
{
...
int PET_data::_read_non_TOF_LST_data(const std::string inputfile_name, int &total_prompt_event, PET_data_type mode)
{
...
int PET_data::_read_TOF_LST_data(const std::string inputfile_name, int &total_prompt_event, PET_data_type mode)
{
	PET_LST_event_type event_mode;
	int event_ptr;
...
int PET_data::_read_TOF_LST_data_CBM(const std::string inputfile_name, int &total_prompt_event, PET_data_type mode)
{
	PET_LST_event_type event_mode;
	int event_ptr;
...
int PET_data::_read_XYZ_coincidence_LST_data(const std::string inputfile_name, int &total_prompt_event, PET_data_type mode)
{
...
void PET_data::CreateFullCoincidenceData_norm(PET_geometry geometry, PET_coincidence_type type, int lower1, int upper1, int lower2, int upper2, float time, int *det1, int *det2)
{
	_create_full_coincidence_LST_data_norm(geometry, time, _num_event, type, lower1, upper1, lower2, upper2, det1, det2);
}

void PET_data::CreateFullCoincidenceData_normblock(PET_geometry geometry, PET_coincidence_type type, int lower1, int upper1, int lower2, int upper2, float time, int **det1)
{
	_create_full_coincidence_LST_data_normblock(geometry, time, _num_event, type, lower1, upper1, lower2, upper2, det1);
}

void PET_data::CreateFullCoincidenceData(PET_geometry geometry, PET_coincidence_type type, int modulePairStart, int modulePairEnd, float time)
{
	if (type == SS)
	{
		cout << "generating SS events" << endl;
		for (int i = modulePairStart; i <= modulePairEnd; i++)
		{
			int dea1 = XE_lookup_A[i - 1];
			int dea2 = XE_lookup_B[i - 1];
			int lower1 = (dea1 - 14 + 19) % 19 * 3200;
			int upper1 = lower1 + 3200;
			int lower2 = (dea2 - 14 + 19) % 19 * 3200;
			int upper2 = lower2 + 3200;
			cout << "generating events between dea " << dea1 << " and dea " << dea2 << endl;
			_create_full_coincidence_LST_data(geometry, time, _num_event, type, lower1, upper1, lower2, upper2);
		}
		_num_ss_event = _num_event;
	}
	if (type == IS)
	{
		cout << "generating OS events" << endl;
		for (int i = modulePairStart; i <= modulePairEnd; i++)
		{
			int dea1 = XE_lookup_A[i - 1];
			int dea2 = XE_lookup_B[i - 1];
			int lower1 = (dea1 - 14 + 19) % 19 * 3200;
			int upper1 = lower1 + 3200;
			int lower2 = NUM_SCANNER_CRYSTALS + NUM_INSERT_CRYSTALS / 2 * (dea2 - 19);
			int upper2 = lower2 + NUM_INSERT_CRYSTALS / 2;
			cout << "generating events between dea " << dea1 << " and dea " << dea2 << endl;
			_create_full_coincidence_LST_data(geometry, time, _num_event, type, lower1, upper1, lower2, upper2);
		}
	}
	if (type == II)
	{
		cout << "generating OO events" << endl;
		int lower1 = NUM_SCANNER_CRYSTALS;
		int upper1 = NUM_SCANNER_CRYSTALS + NUM_INSERT_CRYSTALS / 2;
		int lower2 = NUM_SCANNER_CRYSTALS + NUM_INSERT_CRYSTALS / 2;
		int upper2 = NUM_SCANNER_CRYSTALS + NUM_INSERT_CRYSTALS;
		cout << "crystal 1: " << lower1 << " to " << upper1 << endl;
		cout << "crystal 2: " << lower2 << " to " << upper2 << endl;
		_create_full_coincidence_LST_data(geometry, time, _num_event, type, lower1, upper1, lower2, upper2);
	}
}

void PET_data::CreateFullCoincidenceData(PET_geometry geometry, PET_coincidence_type type, int lower1, int upper1, int lower2, int upper2)
{
	_create_min_coincidence_LST_data(geometry, _num_event, type, lower1, upper1, lower2, upper2);
}
void PET_data::AddScatter(float scale)
{
	int numEvents = PET_LST_event_list.size();
	float meanScatter = 0.0f;
	float maxScatter = 0.0f;
	float minScatter = 1.0f;
	for (int i = 0; i < numEvents; i++)
	{
		PET_LST_event_list[i].random_scatter_correction_factor += scale * scatter_data[i];
		maxScatter = max(maxScatter, scale * (float)scatter_data[i]);
		minScatter = min(maxScatter, scale * (float)scatter_data[i]);
		meanScatter += scale * scatter_data[i] / numEvents;
	}
	cout << "scatter fraction added, using scale " << scale << ", min scatter " << minScatter << ", max scatter " << maxScatter << ", mean scatter " << meanScatter << endl;
}

void PET_data::AddRandom(const std::string delayed_filename, const std::string detector_eff_filename, float scale_random)
{
	float *crystal_efficency = (float *)malloc((NUM_INSERT_CRYSTALS + NUM_SCANNER_CRYSTALS) * sizeof(float));
	float meanRandom = 0.0f;
	float maxRandom = 0.0f;
	float minRandom = 1.0f;
	vector<vector<float>> randomMatrix(336, vector<float>(336));
	ifstream fileRandomMatrix;
	fileRandomMatrix.open(delayed_filename);
	cout << "reading random matrix from" << delayed_filename << endl;
	for (int i = 0; i < 336; i++)
	{
		for (int j = 0; j < 336; j++)
		{
	...
	printf("......%s read successfully.\n", delayed_filename.c_str());

	FILE *crystalEffciencyFile = fopen(detector_eff_filename.c_str(), "rb");
	int t1 = fread(crystal_efficency, sizeof(float), NUM_INSERT_CRYSTALS + NUM_SCANNER_CRYSTALS, crystalEffciencyFile);
	if ((int)t1 == (NUM_INSERT_CRYSTALS + NUM_SCANNER_CRYSTALS))
	{
		printf("......%s read successfully.\n", detector_eff_filename.c_str());
	}
	else
	{
		printf("......%s read error. %d elements were read\n", detector_eff_filename.c_str(), (int)t1);
	}
	...
	{
		int block1, block2;
		int crystal_index_1 = PET_LST_event_list[i].src_id;
		int crystal_index_2 = PET_LST_event_list[i].dest_id;

		float n_crystal_block1, n_crystal_block2;
		if (crystal_index_1 < NUM_SCANNER_CRYSTALS)
		{
			block1 = crystal_index_1 / 200;
			n_crystal_block1 = 200.0f;
		}
		else
		{
			block1 = 304 + (crystal_index_1 - NUM_SCANNER_CRYSTALS) / 1800;
			n_crystal_block1 = 1800.0f;
		}
		if (crystal_index_2 < NUM_SCANNER_CRYSTALS)
		{
			block2 = crystal_index_2 / 200;
			n_crystal_block2 = 200.0f;
		}
		else
		{
			block2 = 304 + (crystal_index_2 - NUM_SCANNER_CRYSTALS) / 1800;
			n_crystal_block2 = 1800.0f;
		}

		PET_LST_event_list[i].random_scatter_correction_factor += scale_random * randomMatrix[block1][block2] / (n_crystal_block1 * n_crystal_block2) 
																* crystal_efficency[crystal_index_1] * crystal_efficency[crystal_index_2]
																* TOF_BIN_SIZE / TOF_WINDOW;
		maxRandom = max(maxRandom, PET_LST_event_list[i].random_scatter_correction_factor);
		minRandom = min(minRandom, PET_LST_event_list[i].random_scatter_correction_factor);
		meanRandom += PET_LST_event_list[i].random_scatter_correction_factor / numEvents;
	}
	cout << "random fraction added, min random " << minRandom << ", max random " << maxRandom << ", mean random " << meanRandom << endl;
}

void PET_data::ReadScatterFromFile(const std::string inputfile_name)
{

	int num_coincidence = PET_LST_event_list.size();
	std::ifstream file(inputfile_name, std::ios::binary);
	if (!file)
	{
	...
	// Calculate the number of double values
	size_t numScatter = size / sizeof(double);
	if (numScatter != num_coincidence)
	{
		cout << "number of scatter fraction "<< numScatter <<" doesn't match number of prompt event " << num_coincidence << endl;
		return;
	}
	...
	{
		scatter_data.push_back(value);
	}
	cout << "scatter data read successfully, scatter data size" << scatter_data.size() << endl;
	file.close();
}

int PET_data::_read_compact_coincidence_LST_data(const std::string inputfile_name, int &total_prompt_event, PET_data_type mode, PET_geometry &detector)
{
	PET_LST_event_type event_mode;
	int event_ptr;
	...
	for (i = 0; i < num_coincidence; i++)
	{
		...
		{
			if (_protocol != STATIC)
				_current_bed_poistion = compact_coincidence_data[i].bed_position - _image_bed_position;
			PET_LST_event current_event(compact_coincidence_data[i].crystal_index_1, compact_coincidence_data[i].crystal_index_2, (float)(1.5e11 * compact_coincidence_data[i].diff_time), (float)(compact_coincidence_data[i].time_1), 0, _current_bed_poistion);
			// in case there are wrong crystal index
			if (compact_coincidence_data[i].crystal_index_1 < 0 || compact_coincidence_data[i].crystal_index_2 < 0 ||
				compact_coincidence_data[i].crystal_index_1 >= NUM_SCANNER_CRYSTALS + NUM_INSERT_CRYSTALS || compact_coincidence_data[i].crystal_index_2 >= NUM_SCANNER_CRYSTALS + NUM_INSERT_CRYSTALS)
				continue;
			PET_LST_event_list.push_back(current_event);
...
void PET_data::ReadPromptAndScatterFromFile(const std::string prompt_filename, const std::string scatter_filename, PET_geometry &detector)
{
	int event_ptr;
	mini_coinc_event *compact_coincidence_data;
	double time_tag;
	double minScatter=0, maxScatter=10000, meanScatter = 0;
	int i;
	struct stat64 file_status;
	long long int size;
	size_t t;
	int num_coincidence;
	FILE *inputfile;
	if (stat64(prompt_filename.c_str(), &file_status) < 0)
	{
		printf("Read file error: %s \n", prompt_filename.c_str());
		return ;
	}
	printf("Size of mini_coinc_event structure is %d\n", (int)sizeof(mini_coinc_event));
	printf("Following coincidence files are being processed:\n");
	...
	if (size >= INT_MAX)
	{
		printf("File \"%s\" length is tooooooooooo long. This program only supports coincidence events fewer than %d. Current file has %lld. Program stopped\n", prompt_filename.c_str(), INT_MAX, size);
		return;
	}
	num_coincidence = (int)size;
	printf("%d coincidence events in file: %s\n", num_coincidence, prompt_filename.c_str());
	...
	// Read in coincidence data
	printf("Opening list mode file, %s \n", prompt_filename.c_str());
	if ((inputfile = fopen(prompt_filename.c_str(), "rb")) == NULL)
	{
		printf("Error: Could not read input file. \n");
		return;
	}
	...
	{
		printf("%s read error. %d elements were read\n", prompt_filename.c_str(), (int)t);
		return;
	}
	fclose(inputfile);

	//
	std::ifstream file(scatter_filename, std::ios::binary);
	if (!file)
	{
		std::cerr << "Cannot open the file!" << std::endl;
		return;
	...
	// Go to the end of the file
	file.seekg(0, std::ios::end);
	size_t size_scatter = file.tellg();
	// Go back to the start of the file
	file.seekg(0, std::ios::beg);

	// Calculate the number of double values
	size_t numScatter = size_scatter / sizeof(double);
	if (numScatter != num_coincidence)
	{
		cout << "number of scatter fraction doesn't match number of prompt event" << endl;
		return;
	}
	// Read the file into the vector
	double value;
	vector<double> scatter_tmp;
	while (file.read(reinterpret_cast<char *>(&value), sizeof(double)))
	{
		scatter_tmp.push_back(value);
	}
	cout << "scatter data read successfully, scatter data size " << scatter_tmp.size() << endl;
	file.close();
	...
	for (i = 0; i < num_coincidence; i++)
	{
		...
		if (MAX_NUM_LST_EVENT > event_ptr && time_tag >= _acq_time_start && time_tag <= _acq_time_end && (int(time_tag) % _acq_time_gap) < 1)
		{
			...
			if (compact_coincidence_data[i].crystal_index_1 < 0 || compact_coincidence_data[i].crystal_index_2 < 0 ||
				...
				continue;
			PET_LST_event_list.push_back(current_event);
			scatter_data.push_back(scatter_tmp[i]);
			minScatter = min(minScatter,scatter_tmp[i]);
			maxScatter = max(minScatter,scatter_tmp[i]);
			meanScatter += scatter_tmp[i];
	...
	_num_event = event_ptr;
	printf("Select time slot from %f seconds to %f seconds with %d seconds gap in between.\n", _acq_time_start, _acq_time_end, _acq_time_gap);
	printf("Total acquisition time is %f seconds.\n", time_tag);
	printf("%d events selected from %d total events.\n", event_ptr, num_coincidence);
	printf("%d scatter fraction, min: %f, max: %f, mean: %f .\n", scatter_data.size(),minScatter,maxScatter,meanScatter/_num_event);
...
int PET_data::_create_full_coincidence_LST_data_norm(PET_geometry &geometry, float &time, int &total_prompt_event, PET_coincidence_type type, int &lower1, int &upper1, int &lower2, int &upper2, int *&det1, int *&det2)
{
...
int PET_data::_create_full_coincidence_LST_data_normblock(PET_geometry &geometry, float &time, int &total_prompt_event, PET_coincidence_type type, int &lower1, int &upper1, int &lower2, int &upper2, int **&det1)
{
	int crystal_index_1, crystal_index_2;
	// double time_tag;
	int i, j;
...
int PET_data::_create_full_coincidence_LST_data(PET_geometry &geometry, float &time, int &total_prompt_event, PET_coincidence_type type, int &lower1, int &upper1, int &lower2, int &upper2)
{
	int crystal_index_1, crystal_index_2;
	// double time_tag;
	int i, j;
	...
	if (type == SS)
	{
		cout << "creating events for SS, max allowed ring difference = " << _MAX_Z_ALLOWED << "mm" << endl;
		for (i = lower1; i < upper1; i++)
		{
			for (j = lower2; j < upper2; j++)
			{
				crystal_index_1 = i;
				crystal_index_2 = j;
				if (abs(geometry.detector_crystal_list[crystal_index_1].geometry.center[2] - geometry.detector_crystal_list[crystal_index_2].geometry.center[2]) <= _MAX_Z_ALLOWED)
				{
...
int PET_data::_create_min_coincidence_LST_data(PET_geometry &geometry, int &total_prompt_event, PET_coincidence_type type, int &lower1, int &upper1, int &lower2, int &upper2)
{
	int crystal_index_1, crystal_index_2;
	int i, j;
	int event_ptr;
...
int PET_data::GetDataCount(PET_coincidence_type type)
{
	if (type == SS)
	{
...
int PET_data::_read_Inveon_LST_data(const std::string inputfile_name, int &total_prompt_event, PET_data_type mode)
{
	...
	while (1)
	{
		if (graycode[gc_counter] == (in_buf[src_pos + 5] & 0xf0))
		{
			...
			case 0x0e: // extended event
				...
				if (pkt_type == selected_event && header_flag)
				{ // timing mode data
					if ((src_pos + 6) > BUFSIZE)
						break;
					...
					if (header_flag || sub_typ1 != 0x01 || sub_typ2 != 0x01)
						break;
					CoinEvtCnt++;
...
int PET_data::synchronize(unsigned int *src_pos_p)
{
...
int PET_data::_get_crystal_id(int sx, int dx, int cx)
{
	return _get_crystal_id_MicroInsert(sx, dx, cx);
}

int PET_data::_get_crystal_id_PlantPET(int sx, int dx, int cx)
{
	int temp;
...
int PET_data::_get_crystal_id_MicroInsert(int sx, int dx, int cx)
{
...
float PET_data::getInitialBedPosition()
{
	return _initial_bed_position;
}

float PET_data::getEndBedPosition()
{
	return _end_bed_position;
}

float PET_data::getBedMOvementStepSize()
{
	return _bed_movement_step_size;
}

float PET_data::getImageBedPosition()
{
	return _image_bed_position;
}
