...
PET_data_scatter::Setup(const std::string config_file_name, PET_data_type type){
	...
	if (_scan_protocol == "CBM"){
		_protocol = CBM;
		cout<<"CBM scan"<<", initial bed position = "<<_initial_bed_position <<endl;

	}
	if (_scan_protocol == "Step and Shoot"){
		_protocol = STEP_AND_SHOOT;
		cout<<"Step and shoot scan, initial bed position = "<<_initial_bed_position <<endl;
	}
	

	cout<<" initial bed position = "<<_initial_bed_position <<endl;
...
PET_data_scatter::_read_compact_coincidence_LST_data(const std::string inputfile_name, int &total_prompt_event, PET_data_type mode, PET_geometry &detector){
	...
	for (i = 0; i<num_coincidence; i++){
		...
		if (MAX_NUM_LST_EVENT > event_ptr && time_tag >= _acq_time_start && time_tag <= _acq_time_end && (int(time_tag) % _acq_time_gap) < 1){
			...
			PET_LST_event_scatter current_event(compact_coincidence_data[i].crystal_index_1, compact_coincidence_data[i].crystal_index_2, (float)(1.5e11 * compact_coincidence_data[i].diff_time), (float)(compact_coincidence_data[i].time_1), _current_bed_poistion,1, compact_coincidence_data[i].scatter_coeff);
