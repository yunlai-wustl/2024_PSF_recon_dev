...
//#define SHIFT_DOI_SCANNER(x)  2.74f  //CRYSTAL_THICKNESS_SCANNER/2-CRYSTAL_WIDTH_SCANNER/2/tanf32(x)
//#define SHIFT_DOI_OUTSERT(x)  3.0f  //CRYSTAL_THICKNESS_OUTSERT/2-CRYSTAL_WIDTH_OUTSERT/2/tanf32(x)


//#define SHIFT_DOI_SCANNER(x)  0.0f  //CRYSTAL_THICKNESS_SCANNER/2-CRYSTAL_WIDTH_SCANNER/2/tanf32(x)
//#define SHIFT_DOI_OUTSERT(x)  0.0f  //CRYSTAL_THICKNESS_OUTSERT/2-CRYSTAL_WIDTH_OUTSERT/2/tanf32(x)


#define SHIFT_DOI_SCANNER(x)  10-(-0.000014076865*x*x*x + 0.00052217937*x*x -0.0181435*x + 7.2787845)
#define SHIFT_DOI_OUTSERT(x)  5-(-0.000011*x*x*x + 0.00074592*x*x -0.0172089*x + 4.28755355)

//***********************************
//**********Edit this part to adapt to different system geometry***************//

#define BlockSize_forward_x 512
#define GridSize_forward_x  704
#define BlockSize_backward_x 512
#define GridSize_backward_x 704

#define BlockSize_forward_y 512
#define GridSize_forward_y 704
#define BlockSize_backward_y 512
#define GridSize_backward_y 704

#define NUM_SUB_SLICE_x 160
#define SIZE_SUB_SLICE_x 5632
#define SUB_SLICE_HEIGHT_x 8

#define NUM_SUB_SLICE_y 160
#define SIZE_SUB_SLICE_y 5632
#define SUB_SLICE_HEIGHT_y 8
	...
	for (sub_slice = 0; sub_slice<NUM_SUB_SLICE_x; sub_slice++){
		...
		for (event_index = threadIdx.x; event_index< num_lines; event_index = event_index + blockDim.x){
			...
			if (src_x<intersection_x - voxel_size_x && dest_x>intersection_x + voxel_size_x){
				...
				solid_angle_ratio = dist_squared;
...
__global__ _TOF_fproj_lst_cuda_x_kernel(float* image, float* fp_x, LST_LORs *_events_x_dominant_uvm, PARAMETERS_IN_DEVICE_t* parameters_device){
	...
	if (threadIdx.x == 0){
		...
		norm_factor = parameters_device->global_norm_factor;
		ptr_src_x = _events_x_dominant_uvm->src_x;
		ptr_src_y = _events_x_dominant_uvm->src_y;
		ptr_src_z = _events_x_dominant_uvm->src_z;
		ptr_dest_x = _events_x_dominant_uvm->dest_x;
	...
	for (sub_slice = 0; sub_slice<NUM_SUB_SLICE_x; sub_slice++){
		...
		for (event_index = threadIdx.x; event_index< num_lines; event_index = event_index + blockDim.x){
			...
			if (src_x<intersection_x - voxel_size_x && dest_x>intersection_x + voxel_size_x){
				...
				//compute the solid angle ratio w.r.t. the location of the intersection
				solid_angle_ratio = 1 + 2 * fabsf(t - 0.5f);
				solid_angle_ratio = dist*dist;				//solid_angle_ratio = 1 * dist*dist;
				...
				for (inslice_z = intersection_index_z - range2; inslice_z <= intersection_index_z + range2; inslice_z++){
					for (inslice_y = intersection_index_y - range1; inslice_y <= intersection_index_y + range1; inslice_y++){
						if (inslice_y >= 0 && inslice_y < num_y && inslice_z >= subslice_z0 && inslice_z < subslice_z1){
							...
							dot_product = projection_vector_y*unit_dir_vector_y + projection_vector_z*unit_dir_vector_z;
							point_line_distance_square = projection_vector_y*projection_vector_y + projection_vector_z*projection_vector_z - dot_product*dot_product;
							kernel_product=__expf(-point_line_distance_square*SIGMA_INV) * ALPHA * t * GLOBAL_SCALE;
							//increment = current_slice_image[(inslice_z - subslice_z0)*num_y + inslice_y] * 0.569;
							increment = current_slice_image[(inslice_z - subslice_z0)*num_y + inslice_y] * kernel_product;
				...
				atomicAdd(&fp_value[event_index], norm_factor*forward_sum/solid_angle_ratio );
...
__global__ _TOF_fproj_lst_cuda_x_kernel_scatter_atten(float* image, float* fp_x, float* atten_per_event, LST_LORs_scatter *_events_x_dominant_uvm, PARAMETERS_IN_DEVICE_t* parameters_device){
	...
	__shared__ float *atten_factor;
	__shared__ float TOF_INV, TOF_ALPHA;
	__shared__ float normlization_factor;
	if (threadIdx.x == 0){
		voxel_size_x = parameters_device->X_SAMP;
		voxel_size_y = parameters_device->Y_SAMP;
		voxel_size_z = parameters_device->Z_SAMP;
		...
		TOF_INV = parameters_device->TOF_inv;
		TOF_ALPHA = parameters_device->TOF_alpha;
		normlization_factor = parameters_device->global_norm_factor;
	...
	for (sub_slice = 0; sub_slice<NUM_SUB_SLICE_x; sub_slice++){
		...
		for (event_index = threadIdx.x; event_index< num_lines; event_index = event_index + blockDim.x){
			...
			if (src_x<intersection_x - voxel_size_x && dest_x>intersection_x + voxel_size_x){
				...
				atomicAdd(&fp_value[event_index], (normlization_factor*forward_sum / solid_angle_ratio));
				//atomicAdd(&fp_value[event_index], forward_sum / solid_angle_ratio);
					...
					for (inslice_z = intersection_index_z - range2; inslice_z <= intersection_index_z + range2; inslice_z++){
						for (inslice_x = intersection_index_x - range1; inslice_x <= intersection_index_x + range1; inslice_x++){
							if (inslice_x >= 0 && inslice_x < num_x && inslice_z >= subslice_z0 && inslice_z < subslice_z1){
								...
								point_line_distance_square = projection_vector_x*projection_vector_x + projection_vector_z*projection_vector_z - dot_product*dot_product;
								kernel_product = __expf(-point_line_distance_square*SIGMA_INV) * ALPHA  * t * GLOBAL_SCALE;
								increment = current_slice_image[(inslice_z - subslice_z0)*num_x + inslice_x] * kernel_product;
							    forward_sum += increment;
...
__global__ _TOF_fproj_lst_cuda_y_kernel_scatter_atten(float* image, float* fp_y, float* atten_per_event, LST_LORs_scatter *_events_y_dominant_uvm, PARAMETERS_IN_DEVICE_t* parameters_device){
	...
	__shared__ float *atten_factor;
	__shared__ float TOF_INV, TOF_ALPHA;
	__shared__ float normalization_factor;
	if (threadIdx.x == 0){
		voxel_size_x = parameters_device->X_SAMP;
		voxel_size_y = parameters_device->Y_SAMP;
		voxel_size_z = parameters_device->Z_SAMP;
		...
		TOF_INV = parameters_device->TOF_inv;
		TOF_ALPHA = parameters_device->TOF_alpha;
		normalization_factor = parameters_device ->global_norm_factor;
		ptr_src_x = _events_y_dominant_uvm->src_x;
		ptr_src_y = _events_y_dominant_uvm->src_y;
		ptr_src_z = _events_y_dominant_uvm->src_z;
		ptr_dest_x = _events_y_dominant_uvm->dest_x;
	...
	for (sub_slice = 0; sub_slice<NUM_SUB_SLICE_y; sub_slice++){
		...
		for (event_index = threadIdx.x; event_index< num_lines; event_index = event_index + blockDim.x){
			...
			if (src_y<intersection_y - voxel_size_y && dest_y>intersection_y + voxel_size_y){
				...
				atomicAdd(&fp_value[event_index], (normalization_factor*forward_sum / solid_angle_ratio));
				//atomicAdd(&fp_value[event_index], forward_sum / solid_angle_ratio);
			}//intersects current slice
		...
		for (event_index = threadIdx.x; event_index< num_lines; event_index = event_index + blockDim.x){
			...
			if (src_x<intersection_x - voxel_size_x && dest_x>intersection_x + voxel_size_x){
				...
				if (fp_value[event_index] < SMALLEST_ALLOWED)
					...
					backward_ratio = 1.0f/fp_value[event_index];
				...
				for (inslice_z = intersection_index_z - range2; inslice_z <= intersection_index_z + range2; inslice_z++){
					for (inslice_y = intersection_index_y - range1; inslice_y <= intersection_index_y + range1; inslice_y++){
						if (inslice_y >= 0 && inslice_y<num_y && inslice_z >= subslice_z0 && inslice_z<subslice_z1){
							...
							atomicAdd(&current_slice_image[(inslice_z - subslice_z0)*num_y + inslice_y] ,( norm_factor*weight/solid_angle_ratio ));
						}
					}
				}
			}//intersects current slice
				...
				for (inslice_z = intersection_index_z - range2; inslice_z <= intersection_index_z + range2; inslice_z++){
					for (inslice_x = intersection_index_x - range1; inslice_x <= intersection_index_x + range1; inslice_x++){
						if (inslice_x >= 0 && inslice_x<num_x && inslice_z >= subslice_z0 && inslice_z<subslice_z1){
							...
							weight = t * backward_ratio * __expf(-point_line_distance_square*SIGMA_INV) * ALPHA * GLOBAL_SCALE;
							//weight = backward_ratio * __expf(-point_line_distance_square*SIGMA_INV) * ALPHA * t;
							atomicAdd(&current_slice_image[(inslice_z - subslice_z0)*num_x + inslice_x], (norm_factor*weight/solid_angle_ratio));
						}
					}
				}
			}//intersects current slice
...
__global__ _TOF_b_ratio_proj_lst_cuda_x_kernel_atten(float* image, float* fp_x, float* atten_per_event, LST_LORs *_events_x_dominant_uvm, PARAMETERS_IN_DEVICE_t* parameters_device){
	...
	__shared__ float *atten_factor;
	__shared__ float TOF_INV, TOF_ALPHA;
	__shared__ float global_norm_factor;
	if (threadIdx.x == 0){
		voxel_size_x = parameters_device->X_SAMP;
		voxel_size_y = parameters_device->Y_SAMP;
		voxel_size_z = parameters_device->Z_SAMP;
		...
		TOF_INV = parameters_device->TOF_inv;
		TOF_ALPHA = parameters_device->TOF_alpha;
		global_norm_factor = parameters_device -> global_norm_factor;
		ptr_src_x = _events_x_dominant_uvm->src_x;
		ptr_src_y = _events_x_dominant_uvm->src_y;
		ptr_src_z = _events_x_dominant_uvm->src_z;
		ptr_dest_x = _events_x_dominant_uvm->dest_x;
	...
	for (sub_slice = 0; sub_slice<NUM_SUB_SLICE_x; sub_slice++){
		...
		for (event_index = threadIdx.x; event_index< num_lines; event_index = event_index + blockDim.x){
			...
			if (src_x<intersection_x - voxel_size_x && dest_x>intersection_x + voxel_size_x){
				...
				if (global_norm_factor*fp_value[event_index]+coeff < SMALLEST_ALLOWED)
					backward_ratio = 0;
				else
					backward_ratio = __frcp_rn(global_norm_factor*fp_value[event_index]+coeff);
				...
				for (inslice_z = intersection_index_z - range2; inslice_z <= intersection_index_z + range2; inslice_z++){
					for (inslice_y = intersection_index_y - range1; inslice_y <= intersection_index_y + range1; inslice_y++){
						if (inslice_y >= 0 && inslice_y<num_y && inslice_z >= subslice_z0 && inslice_z<subslice_z1){
							...
							atomicAdd(&current_slice_image[(inslice_z - subslice_z0)*num_y + inslice_y],global_norm_factor*(weight / solid_angle_ratio));
						}
					}
				}
			}//intersects current slice
				...
				for (inslice_z = intersection_index_z - range2; inslice_z <= intersection_index_z + range2; inslice_z++){
					for (inslice_x = intersection_index_x - range1; inslice_x <= intersection_index_x + range1; inslice_x++){
						if (inslice_x >= 0 && inslice_x<num_x && inslice_z >= subslice_z0 && inslice_z<subslice_z1){
							...
							point_line_distance_square = projection_vector_x*projection_vector_x + projection_vector_z*projection_vector_z - dot_product*dot_product;
							weight = t * backward_ratio * __expf(-point_line_distance_square*SIGMA_INV - integral_mu) * ALPHA * GLOBAL_SCALE;
							atomicAdd(&current_slice_image[(inslice_z - subslice_z0)*num_x + inslice_x], global_norm_factor*(weight/ solid_angle_ratio));
						}
					}
				}
			}//intersects current slice
				...
				for (inslice_z = intersection_index_z - range2; inslice_z <= intersection_index_z + range2; inslice_z++){
					for (inslice_y = intersection_index_y - range1; inslice_y <= intersection_index_y + range1; inslice_y++){
						if (inslice_y >= 0 && inslice_y<num_y && inslice_z >= subslice_z0 && inslice_z<subslice_z1){
							...
							point_line_distance_square = projection_vector_y*projection_vector_y + projection_vector_z*projection_vector_z - dot_product*dot_product;
							weight = t * backward_ratio * __expf(-point_line_distance_square*SIGMA_INV - integral_mu) * ALPHA * GLOBAL_SCALE;
							atomicAdd(&current_slice_image[(inslice_z - subslice_z0)*num_y + inslice_y], normalization_factor*(weight) / solid_angle_ratio);
						}
					}
				}
			}//intersects current slice
				...
				for (inslice_z = intersection_index_z - range2; inslice_z <= intersection_index_z + range2; inslice_z++){
					for (inslice_x = intersection_index_x - range1; inslice_x <= intersection_index_x + range1; inslice_x++){
						if (inslice_x >= 0 && inslice_x<num_x && inslice_z >= subslice_z0 && inslice_z<subslice_z1){
							...
							point_line_distance_square = projection_vector_x*projection_vector_x + projection_vector_z*projection_vector_z - dot_product*dot_product;
							weight = t * backward_ratio * __expf(-point_line_distance_square*SIGMA_INV - integral_mu) * ALPHA * GLOBAL_SCALE;
							atomicAdd(&current_slice_image[(inslice_z - subslice_z0)*num_x + inslice_x], normalization_factor*(weight) / solid_angle_ratio);
						}
					}
				}
			}//intersects current slice
	...
	for (sub_slice = 0; sub_slice<NUM_SUB_SLICE_x; sub_slice++){
		...
		for (event_index = threadIdx.x; event_index< num_lines; event_index = event_index + blockDim.x){
			...
			if (src_x<intersection_x - voxel_size_x && dest_x>intersection_x + voxel_size_x){
				...
				//compute the solid angle ratio w.r.t. the location of the intersection
				solid_angle_ratio = 1 + 2 * fabsf(t - 0.5f);
				solid_angle_ratio = dist*dist;				//solid_angle_ratio = 1 + 12 * (t - 0.5f) * (t - 0.5f);
...
cuda_em_recon::_Initialize_host_parameters(parameters_t p){
	...
	parameters_host.global_norm_factor_ss = p.global_norm_factor_ss;
	parameters_host.global_norm_factor_os = p.global_norm_factor_os;
	parameters_host.global_norm_factor_oo = p.global_norm_factor_oo;
...
//	parameters_host.FWHM_alpha = 2 * sqrt(log(2) / M_PI) / LOR_FWHM;
	parameters_host.FWHM_alpha = 4 * (log(2) / M_PI) / (LOR_FWHM*LOR_FWHM);
	...
	parameters_host.FWHM_alpha_ss = 4 * (log(2) / M_PI) / (LOR_FWHM*LOR_FWHM);
	...
	parameters_host.FWHM_alpha_is = 4 * (log(2) / M_PI) / (LOR_FWHM*LOR_FWHM);
...
//	parameters_host.FWHM_alpha_ii = 2 * sqrt(log(2) / M_PI) / LOR_FWHM;
	parameters_host.FWHM_alpha_ii = 4 * (log(2) / M_PI) / (LOR_FWHM*LOR_FWHM);
...
// 	float *scatter_random_x, *scatter_random_y, *coeff_z;
...
// 	scatter_random_x = (float*)malloc(total_event_count*sizeof(float));
...
// 	scatter_random_y = (float*)malloc(total_event_count*sizeof(float));
...
// 		float bed_shift_dist = current_lst_event.bed_position;
// 		//float bed_shift_dist = 0.0;
// 		//cout << "\n current_lst_event.t0: " << current_lst_event.t0 << " Shift_dist : " << bed_shift_dist;
...
// 		temp_s.center[2] = detector.detector_crystal_list.at(current_lst_event.src_id).geometry.center[2] - bed_shift_dist;
...
// 		temp_d.center[2] = detector.detector_crystal_list.at(current_lst_event.dest_id).geometry.center[2] - bed_shift_dist;
...
// 			scatter_random_x[event_count_x_dominant] = coeffx ;
...
// 			}
// 			scatter_random_y[event_count_y_dominant] = coeffy ;
...
// 	checkCudaErrors(cudaMemcpy(events_x_host.coeff, scatter_random_x, events_x_host.num_lines*sizeof(float), cudaMemcpyDefault));
...
// 	checkCudaErrors(cudaMemcpy(events_y_host.coeff, scatter_random_y, events_y_host.num_lines*sizeof(float), cudaMemcpyDefault));
...
cuda_em_recon::_Mem_allocation_for_LST_events_memcopy(PET_geometry& detector, PET_movement& movement, PET_data& data, int start_index, int end_index, PET_coincidence_type pet_coinc_type, LST_LORs*& device_events_x_global_mem, LST_LORs*& device_events_y_global_mem, int& event_count_x_dominant, int& event_count_y_dominant, int device_ID){
	...
	float *src_x_z, *src_y_z, *src_z_z, *dest_x_z, *dest_y_z, *dest_z_z;
	float *scatter_random_x, *scatter_random_y, *coeff_z;
	float max_scatter_random_x=0, min_scatter_random_x=1,mean_scatter_random_x=0;
	float max_scatter_random_y=0, min_scatter_random_y=1,mean_scatter_random_y=0;
	float snorm_x, dnorm_x, centm_x, snorm_z, dnorm_z, centm_z, angle_s_x, angle_d_x, cos_s_x, cos_d_x;
	float FWHM_s, FWHM_d; //FWHM of Detector response function of destination and sourse crystal
	float alpha_x, alpha_y, sin_incident_angle_xy;
	float incident_angle_xy_src,incident_angle_xy_dest;
	float snorm_y, dnorm_y, centm_y, angle_s_y, angle_d_y, angle_s_z, angle_d_z, cos_s_y, cos_d_y, cos_s_z, cos_d_z, coeff_s_z, coeff_d_z;
	float DOI_shift_dist_src,DOI_shift_dist_dest;
	int position;
	box temp_s, temp_d;
		...
		*/
		...
		if (fabs(dir[0]) >= fabs(dir[1])){
			...
			if (s.center[0] <= d.center[0]){			
				...
				angle_d_x = acos((cos_d_x)) * 180 / M_PI;
				/*
				sin_incident_angle_xy = abs( (d.center[0] - s.center[0])*s.normal_1[1]- (d.center[1] - s.center[1])* s.normal_1[0] )
				...
				*/
				if(current_lst_event.src_id < NUM_SCANNER_CRYSTALS)
					DOI_shift_dist_src = max(0.0f,SHIFT_DOI_SCANNER(angle_s_x));
				else
					DOI_shift_dist_src = max(0.0f,SHIFT_DOI_OUTSERT(angle_s_x));
				if(current_lst_event.dest_id < NUM_SCANNER_CRYSTALS)
					DOI_shift_dist_dest = max(0.0f,SHIFT_DOI_SCANNER(angle_d_x));
				else
					DOI_shift_dist_dest = max(0.0f,SHIFT_DOI_OUTSERT(angle_d_x));
				
				if ((angle_s_x >= 0 && angle_s_x <= 75) && (angle_d_x >= 0 && angle_d_x <= 75)){
					alpha_x=atan2f32((d.center[1]+s.center[1])/2,(d.center[0]+s.center[0])/2);
					src_x_x[event_count_x_dominant] = s.center[0]+DOI_shift_dist_src*s.normal_1[0];
					src_y_x[event_count_x_dominant] = s.center[1]+DOI_shift_dist_src*s.normal_1[1];
					src_z_x[event_count_x_dominant] = s.center[2]+DOI_shift_dist_src*s.normal_1[2];
					dest_x_x[event_count_x_dominant] = d.center[0]+DOI_shift_dist_dest*d.normal_1[0];
					dest_y_x[event_count_x_dominant] = d.center[1]+DOI_shift_dist_dest*d.normal_1[1];
					dest_z_x[event_count_x_dominant] = d.center[2]+DOI_shift_dist_dest*d.normal_1[2];
					TOF_dist_x[event_count_x_dominant] = TOF_dist_temp;
				...
				if ((angle_s_x >= 0 && angle_s_x <= 75) && (angle_d_x >= 0 && angle_d_x <= 75)){
						//NOTE!!!! When exchange the order of src and dest, be sure to flip the sign of TOF
						src_x_x[event_count_x_dominant] = d.center[0] + DOI_shift_dist_dest*d.normal_1[0];
						src_y_x[event_count_x_dominant] = d.center[1] + DOI_shift_dist_dest*d.normal_1[1];
						src_z_x[event_count_x_dominant] = d.center[2] + DOI_shift_dist_dest*d.normal_1[2];
						dest_x_x[event_count_x_dominant] = s.center[0]+ DOI_shift_dist_src*s.normal_1[0];
						dest_y_x[event_count_x_dominant] = s.center[1]+ DOI_shift_dist_src*s.normal_1[1];
						dest_z_x[event_count_x_dominant] = s.center[2]+ DOI_shift_dist_src*s.normal_1[2];
						TOF_dist_x[event_count_x_dominant] = -TOF_dist_temp;
					alpha_x=atan2f32((d.center[1]+s.center[1])/2,(d.center[0]+s.center[0])/2);
					//if(event_count_x_dominant<50)
					//	cout <<"d.center[1]:"<<d.center[1]<<", s.center[1]"<<s.center[1]<<", d.center[0]"<<d.center[0]<<", s.center[0]"<<s.center[0] <<", alpha_x:" <<alpha_x<<"\n";
			...
			scatter_random_x[event_count_x_dominant] = current_lst_event.random_scatter_correction_factor ;
			max_scatter_random_x=max(max_scatter_random_x,current_lst_event.random_scatter_correction_factor);
			min_scatter_random_x=min(min_scatter_random_x,current_lst_event.random_scatter_correction_factor);
			mean_scatter_random_x += current_lst_event.random_scatter_correction_factor;
			event_count_x_dominant++;
		}
		else if (fabs(dir[1]) > fabs(dir[0])){
			//vertical, predominant = 1
			...
			if (s.center[1] <= d.center[1]){			
					...
					if(current_lst_event.src_id < NUM_SCANNER_CRYSTALS)
						DOI_shift_dist_src = max(0.0f,SHIFT_DOI_SCANNER(angle_s_y));
					else
						DOI_shift_dist_src = max(0.0f,SHIFT_DOI_OUTSERT(angle_s_y));
					if(current_lst_event.dest_id < NUM_SCANNER_CRYSTALS)
						DOI_shift_dist_dest = max(0.0f,SHIFT_DOI_SCANNER(angle_d_y));
					else
						DOI_shift_dist_dest = max(0.0f,SHIFT_DOI_OUTSERT(angle_d_y));
					if ((angle_s_y >= 0 && angle_s_y <= 75) && (angle_d_y >= 0 && angle_d_y <= 75)){
						src_x_y[event_count_y_dominant] = s.center[0]+DOI_shift_dist_src*s.normal_1[0];
						src_y_y[event_count_y_dominant] = s.center[1]+DOI_shift_dist_src*s.normal_1[1];
						src_z_y[event_count_y_dominant] = s.center[2]+DOI_shift_dist_src*s.normal_1[2];
						dest_x_y[event_count_y_dominant] = d.center[0]+DOI_shift_dist_dest*d.normal_1[0];
						dest_y_y[event_count_y_dominant] = d.center[1]+DOI_shift_dist_dest*d.normal_1[1];
						dest_z_y[event_count_y_dominant] = d.center[2]+DOI_shift_dist_dest*d.normal_1[2];
						TOF_dist_y[event_count_y_dominant] = TOF_dist_temp;
				...
				if(current_lst_event.dest_id < NUM_SCANNER_CRYSTALS)
					...
					DOI_shift_dist_dest = max(0.0f,SHIFT_DOI_OUTSERT(angle_d_y));
				if ((angle_s_y >= 0 && angle_s_y <= 75) && (angle_d_y >= 0 && angle_d_y <= 75)){
					src_x_y[event_count_y_dominant] = d.center[0]+DOI_shift_dist_dest*d.normal_1[0];
					src_y_y[event_count_y_dominant] = d.center[1]+DOI_shift_dist_dest*d.normal_1[1];
					src_z_y[event_count_y_dominant] = d.center[2]+DOI_shift_dist_dest*d.normal_1[2];
					dest_x_y[event_count_y_dominant] = s.center[0]+DOI_shift_dist_src*s.normal_1[0];
					dest_y_y[event_count_y_dominant] = s.center[1]+DOI_shift_dist_src*s.normal_1[1];
					dest_z_y[event_count_y_dominant] = s.center[2]+DOI_shift_dist_src*s.normal_1[2];
					//NOTE!!!! When exchange the order of src and dest, be sure to flip the sign of TOF
					TOF_dist_y[event_count_y_dominant] = -TOF_dist_temp;
					alpha_y=atan2f32((d.center[1]+s.center[1])/2,(d.center[0]+s.center[0])/2);
				}
			...
			scatter_random_y[event_count_y_dominant] = current_lst_event.random_scatter_correction_factor ;
			max_scatter_random_y=max(max_scatter_random_y,current_lst_event.random_scatter_correction_factor);
			min_scatter_random_y=min(min_scatter_random_y,current_lst_event.random_scatter_correction_factor);
			mean_scatter_random_y += current_lst_event.random_scatter_correction_factor;
			event_count_y_dominant++;
		}
	}
	LST_LORs events_x_host;
	...
	cout << "Total number of x-predominant event : " << event_count_x_dominant << std::endl;
	cout << "Total number of y-predominant event : " << event_count_y_dominant << std::endl;
	cout << "Min scatter + random fraction, x "<<min_scatter_random_x <<", y " <<min_scatter_random_y <<endl;
	cout << "Max scatter + random fraction, x "<<max_scatter_random_x <<", y " <<max_scatter_random_y <<endl;
	if(event_count_x_dominant!=0)
		cout << "Mean scatter + random fraction, x "<<mean_scatter_random_x/event_count_x_dominant;
	else
		cout << "Mean scatter + random fraction, x 0";
	if(event_count_y_dominant!=0)
		cout <<", y " <<mean_scatter_random_y/event_count_y_dominant <<endl;
	else
		cout << "Mean scatter + random fraction, y 0 "<<endl;
...
cuda_em_recon::_Mem_allocation_for_LST_events_memcopy_scatter(PET_geometry& detector, PET_movement& movement, PET_data_scatter& data, int start_index, int end_index, PET_coincidence_type pet_coinc_type, LST_LORs_scatter*& device_events_x_global_mem, LST_LORs_scatter*& device_events_y_global_mem, int& event_count_x_dominant, int& event_count_y_dominant, int device_ID){
	...
	float *src_x_y, *src_y_y, *src_z_y, *dest_x_y, *dest_y_y, *dest_z_y;
	float *src_x_z, *src_y_z, *src_z_z, *dest_x_z, *dest_y_z, *dest_z_z;
	int *scatter_random_x, *scatter_random_y, *coeff_z;
	float *sc_fraction_x, *sc_fraction_y;
	float snorm_x, dnorm_x, centm_x, snorm_z, dnorm_z, centm_z, angle_s_x, angle_d_x, cos_s_x, cos_d_x;
	int coeffx, coeffy;
	float snorm_y, dnorm_y, centm_y, angle_s_y, angle_d_y, angle_s_z, angle_d_z, cos_s_y, cos_d_y, cos_s_z, cos_d_z, coeff_s_z, coeff_d_z;
	...
	dest_y_x = (float*)malloc(total_event_count*sizeof(float));
	dest_z_x = (float*)malloc(total_event_count*sizeof(float));
	scatter_random_x = (int*)malloc(total_event_count*sizeof(int));
	sc_fraction_x = (float*)malloc(total_event_count*sizeof(float));
	...
	dest_y_y = (float*)malloc(total_event_count*sizeof(float));
	dest_z_y = (float*)malloc(total_event_count*sizeof(float));
	scatter_random_y = (int*)malloc(total_event_count*sizeof(int));
	sc_fraction_y = (float*)malloc(total_event_count*sizeof(float));
	...
	for (lst_event_index = start_index; lst_event_index <= end_index; lst_event_index++){
		...
		//cout << "\n current_lst_event.t0: " << current_lst_event.t0 << " timestep: " << timestep << " diststep: " << diststep<<" Shift_dist : " << bed_shift_dist;
		...
		if (fabs(dir[0]) >= fabs(dir[1])){
			...
			if (s.center[0] <= d.center[0]){
				...
				if ((angle_s_x >= 0 && angle_s_x <= 75) && (angle_d_x >= 0 && angle_d_x <= 75)){
					...
					dest_z_x[event_count_x_dominant] = d.center[2]+DOI_shift_dist_dest*d.normal_1[2];
					TOF_dist_x[event_count_x_dominant] = TOF_dist_temp;
					sc_fraction_x[event_count_x_dominant] = sc_coeff;
					//cout << angle_s_x << " " << angle_d_x <<" " << cos_s_x<< " " << cos_d_x<< " " <<centm_x<<"\n";
				}
				else{
					coeffx = 0;
			...
			if (s.center[1] <= d.center[1]){
				...
				if ((angle_s_y >= 0 && angle_s_y <= 75) && (angle_d_y >= 0 && angle_d_y <= 75)){				
					...
					dest_z_y[event_count_y_dominant] = d.center[2]+DOI_shift_dist_dest*d.normal_1[2];
					TOF_dist_y[event_count_y_dominant] = TOF_dist_temp;
					sc_fraction_y[event_count_y_dominant] = sc_coeff;
					//cout << angle_s_y << " " << angle_d_y << " " << cos_s_y << " " << cos_d_y << " " << centm_y << "\n";
				}
				else{
					coeffy = 0;
	...
	checkCudaErrors(cudaMemcpy(events_x_host.dest_y, dest_y_x, events_x_host.num_lines*sizeof(float), cudaMemcpyDefault));
	checkCudaErrors(cudaMemcpy(events_x_host.dest_z, dest_z_x, events_x_host.num_lines*sizeof(float), cudaMemcpyDefault));
	checkCudaErrors(cudaMemcpy(events_x_host.coeff, scatter_random_x, events_x_host.num_lines*sizeof(int), cudaMemcpyDefault));
	checkCudaErrors(cudaMemcpy(events_x_host.TOF_dist, TOF_dist_x, events_x_host.num_lines*sizeof(float), cudaMemcpyDefault));
	checkCudaErrors(cudaMemcpy(events_x_host.sc_coeff, sc_fraction_x, events_x_host.num_lines*sizeof(float), cudaMemcpyDefault));
	...
	checkCudaErrors(cudaMemcpy(events_y_host.dest_y, dest_y_y, events_y_host.num_lines*sizeof(float), cudaMemcpyDefault));
	checkCudaErrors(cudaMemcpy(events_y_host.dest_z, dest_z_y, events_y_host.num_lines*sizeof(float), cudaMemcpyDefault));
	checkCudaErrors(cudaMemcpy(events_y_host.coeff, scatter_random_y, events_y_host.num_lines*sizeof(int), cudaMemcpyDefault));
	checkCudaErrors(cudaMemcpy(events_y_host.TOF_dist, TOF_dist_y, events_y_host.num_lines*sizeof(float), cudaMemcpyDefault));
	checkCudaErrors(cudaMemcpy(events_y_host.sc_coeff, sc_fraction_y, events_y_host.num_lines*sizeof(float), cudaMemcpyDefault));
...
cuda_em_recon::_Mem_allocation_for_LST_events_memcopy_norm(PET_geometry& detector, PET_movement& movement, PET_data& data, int start_index, int end_index, PET_coincidence_type pet_coinc_type, LST_LORs*& device_events_x_global_mem, LST_LORs*& device_events_y_global_mem, int& event_count_x_dominant, int& event_count_y_dominant, int device_ID){
	...
	for (lst_event_index = start_index; lst_event_index <= end_index; lst_event_index++){
		...
		int diststep = movement.get_dist_step();
		int current_time = current_lst_event.t0;
		int bed_shift_dist = (current_time / timestep)*diststep;
		...
		TOF_dist_temp = current_lst_event.TOF_dist;
		//count = current_lst_event.random_scatter_correction_factor;
		...
		*/
		...
		if (fabs(dir[0]) >= fabs(dir[1])){
			...
			scatter_random_x[event_count_x_dominant] = coeffx;
			//cout << "\n" << scatter_random_x[event_count_x_dominant];
			event_count_x_dominant++;
			...
			scatter_random_y[event_count_y_dominant] = coeffy;
			//cout << "\n" << scatter_random_y[event_count_y_dominant];
...
cuda_em_recon::ComputeUpdateFactor(PET_geometry& detector, PET_movement& movement, PET_data& data, ImageArray<float>& Image, ImageArray<float>& UpdateFactor, ImageArray<float>& Attenuation_Image, float &nloglikelihood){
	...
	//SS events
		...
		//copy data to all GPUs
	device_data_length = data.GetDataCount(SS) / (_num_gpu_end - _num_gpu_start + 1);
	...
	for (device_id = _num_gpu_start; device_id <= _num_gpu_end; device_id++){
		...
		if (device_id < _num_gpu_end){
			...
			std::cout << "Copying data from " << data_part_index*device_data_length << " to " << data.GetDataCount(SS) << " to device " << device_id << std::endl;
			Setup_data(detector, movement, data, data_part_index*device_data_length, data.GetDataCount(SS) - 1, SS, device_id);
		}
	}
		...
		//ofstream output_file_fpx("fp_x.txt");
		//ofstream output_file_fpy("fp_y.txt");
		/*
		for (device_id = _num_gpu_start; device_id <= _num_gpu_end; device_id++){
			...
			cudaDeviceSynchronize();
			cout<<"device "<< device_id << ", count of y event:"<<size1<<endl;
			cout<<"device "<< device_id << ", count of x event:"<<size2<<endl;
			for (int i = 0; i < size1; i++){
				//output_file_fpy << fpvalue1_hosty[i] <<endl;
			...
			for (int j = 0; j < size2; j++){
				//output_file_fpx << fpvalue1_hostx[j] <<endl;
			...
			cout << "\n Sum fp for device " << device_id << " : " << sum_fp << " Mean fp : " << mean_fp<<" Max fp : "<<max_fp<<endl;
			
			ostringstream fileNameY;
			fileNameY << "SS_fp_y_" << device_id << ".bin";
			std::ofstream outFileY(fileNameY.str(), std::ios::binary | std::ios::out);
			if (!outFileY.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hosty." << std::endl;
    			// Handle the error (e.g., by exiting the function or the program)
    			return; // or exit(1);
			}
			outFileY.write(reinterpret_cast<const char*>(fpvalue1_hosty), size1 * sizeof(float));
			outFileY.close();

			// Write fpvalue1_hostx to a separate file
			ostringstream fileNameX;
			fileNameX << "SS_fp_x_" << device_id << ".bin";
			std::ofstream outFileX(fileNameX.str(), std::ios::binary | std::ios::out);
			if (!outFileX.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hostx." << std::endl;
    			// Handle the error
    			return; // or exit(1);
			}
			outFileX.write(reinterpret_cast<const char*>(fpvalue1_hostx), size2 * sizeof(float));
			outFileX.close();

			// Free the dynamically allocated memory
			delete[] fpvalue1_hosty;
			delete[] fpvalue1_hostx;
		}
		...
		for (device_id = _num_gpu_start; device_id <= _num_gpu_end; device_id++){
			...
			ostringstream fileNameY;
			fileNameY << "SS_fp_att_y_" << device_id << ".bin";
			std::ofstream outFileY(fileNameY.str(), std::ios::binary | std::ios::out);
			if (!outFileY.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hosty." << std::endl;
    			// Handle the error (e.g., by exiting the function or the program)
			...
			outFileY.write(reinterpret_cast<const char*>(fpvalue2_hosty), size1 * sizeof(float));
			outFileY.close();

			// Write fpvalue1_hostx to a separate file
			ostringstream fileNameX;
			fileNameX << "SS_fp_att_x_" << device_id << ".bin";
			std::ofstream outFileX(fileNameX.str(), std::ios::binary | std::ios::out);
			if (!outFileX.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hostx." << std::endl;
    			// Handle the error
			...
			outFileX.write(reinterpret_cast<const char*>(fpvalue2_hostx), size2 * sizeof(float));
			outFileX.close();

			// Free the dynamically allocated memory
			delete[] fpvalue2_hosty;
			delete[] fpvalue2_hostx;
		}
		*/
	...
	int ind_os_start = data.GetDataCount(SS);
	device_data_length = data.GetDataCount(IS) / (_num_gpu_end - _num_gpu_start + 1);
	for (device_id = _num_gpu_start; device_id <= _num_gpu_end; device_id++){
		data_part_index = device_id - _num_gpu_start;
		if (device_id < _num_gpu_end){
			std::cout << "Copying data from " << ind_os_start+data_part_index*device_data_length << " to " << ind_os_start+(data_part_index + 1)*device_data_length - 1 << " to device " << device_id << std::endl;
			Setup_data(detector, movement, data, ind_os_start+data_part_index*device_data_length, ind_os_start+(data_part_index + 1)*device_data_length - 1, IS, device_id);
		}
		else{
			std::cout << "Copying data from " << ind_os_start+data_part_index*device_data_length << " to " << data.GetDataCount(SS)+ data.GetDataCount(IS) - 1 << " to device " << device_id << std::endl;
			Setup_data(detector, movement, data, ind_os_start+data_part_index*device_data_length, data.GetDataCount(SS)+ data.GetDataCount(IS) - 1, IS, device_id);
		}
	}
		...
		parameters_host.global_norm_factor = parameters_host.global_norm_factor_os;
		...
		for (device_id = _num_gpu_start; device_id <= _num_gpu_end; device_id++){
			...
			ostringstream fileNameY;
			fileNameY << "OS_fp_y_" << device_id << ".bin";
			std::ofstream outFileY(fileNameY.str(), std::ios::binary | std::ios::out);
			if (!outFileY.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hosty." << std::endl;
    			// Handle the error (e.g., by exiting the function or the program)
			...
			// Write fpvalue1_hostx to a separate file
			ostringstream fileNameX;
			fileNameX << "OS_fp_x_" << device_id << ".bin";
			std::ofstream outFileX(fileNameX.str(), std::ios::binary | std::ios::out);
			if (!outFileX.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hostx." << std::endl;
    			// Handle the error
			...
			ostringstream fileNameY;
			fileNameY << "OS_fp_att_y_" << device_id << ".bin";
			std::ofstream outFileY(fileNameY.str(), std::ios::binary | std::ios::out);
			if (!outFileY.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hosty." << std::endl;
    			// Handle the error (e.g., by exiting the function or the program)
			...
			// Write fpvalue1_hostx to a separate file
			ostringstream fileNameX;
			fileNameX << "OS_fp_att_x_" << device_id << ".bin";
			std::ofstream outFileX(fileNameX.str(), std::ios::binary | std::ios::out);
			if (!outFileX.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hostx." << std::endl;
    			// Handle the error
		...
		*/
		...
		int ind_oo_start = data.GetDataCount(SS)+ data.GetDataCount(IS);
		device_data_length = data.GetDataCount(II) / (_num_gpu_end - _num_gpu_start + 1);
		for (device_id = _num_gpu_start; device_id <= _num_gpu_end; device_id++){
			data_part_index = device_id - _num_gpu_start;
			if (device_id < _num_gpu_end){
				std::cout << "Copying data from " << ind_oo_start+data_part_index*device_data_length << " to " << ind_oo_start+(data_part_index + 1)*device_data_length - 1 << " to device " << device_id << std::endl;
				Setup_data(detector, movement, data, ind_oo_start+data_part_index*device_data_length, ind_oo_start+(data_part_index + 1)*device_data_length - 1, II, device_id);
			}
			else{
				std::cout << "Copying data from " << ind_oo_start+data_part_index*device_data_length << " to " <<  data.GetDataCount(ALL) - 1 << " to device " << device_id << std::endl;
				Setup_data(detector, movement, data, ind_oo_start+data_part_index*device_data_length, data.GetDataCount(ALL) - 1, II, device_id);
			}
		}
		...
		parameters_host.global_norm_factor = parameters_host.global_norm_factor_oo;
		...
		for (device_id = _num_gpu_start; device_id <= _num_gpu_end; device_id++){
			...
			ostringstream fileNameY;
			fileNameY << "OO_fp_y_" << device_id << ".bin";
			std::ofstream outFileY(fileNameY.str(), std::ios::binary | std::ios::out);
			if (!outFileY.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hosty." << std::endl;
    			// Handle the error (e.g., by exiting the function or the program)
			...
			// Write fpvalue1_hostx to a separate file
			ostringstream fileNameX;
			fileNameX << "OO_fp_x_" << device_id << ".bin";
			std::ofstream outFileX(fileNameX.str(), std::ios::binary | std::ios::out);
			if (!outFileX.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hostx." << std::endl;
    			// Handle the error
		...
		for (device_id = _num_gpu_start; device_id <= _num_gpu_end; device_id++){
			...
			ostringstream fileNameY;
			fileNameY << "OO_fp_att_y_" << device_id << ".bin";
			std::ofstream outFileY(fileNameY.str(), std::ios::binary | std::ios::out);
			if (!outFileY.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hosty." << std::endl;
    			// Handle the error (e.g., by exiting the function or the program)
			...
			// Write fpvalue1_hostx to a separate file
			ostringstream fileNameX;
			fileNameX << "OO_fp_att_x_" << device_id << ".bin";
			std::ofstream outFileX(fileNameX.str(), std::ios::binary | std::ios::out);
			if (!outFileX.is_open()) {
    			std::cerr << "Failed to open the file for fpvalue1_hostx." << std::endl;
    			// Handle the error
...
cuda_em_recon::ComputeUpdateFactorScatter(PET_geometry& detector, PET_movement& movement, PET_data_scatter& data, ImageArray<float>& Image, ImageArray<float>& UpdateFactor, ImageArray<float>& Attenuation_Image, float &nloglikelihood){
	...
	printf("global normalization factor = %f \n",parameters_host.global_norm_factor);
	printf("H_CENTER_X = %f H_CENTER_Y = %f V_CENTER = %f\n", parameters_host.H_CENTER_X, parameters_host.H_CENTER_Y, parameters_host.V_CENTER);
	printf("RANGE1 = %d RANGE2 = %d FWHM_SS = %f\n", parameters_host.RANGE1, parameters_host.RANGE2, parameters_host.FWHM_ss);
	cudaHostRegister(&parameters_host, sizeof(PARAMETERS_IN_DEVICE_t), 0);
	...
	for (device_id = _num_gpu_start; device_id <= _num_gpu_end; device_id++){
		...
		float* fpvalue1_hosty = new float[size1]();
		float* fpvalue1_hostx = new float[size2]();
		//auto* _event_y = new LST_LORs_scatter[size1]();
		//auto* _event_x = new LST_LORs_scatter[size2]();
		...
		//cudaMemcpy(_event_y,_events_y_dominant_global_mem[device_id],size1*sizeof(LST_LORs_scatter),cudaMemcpyDefault);
		//cudaMemcpy(_event_x,_events_x_dominant_global_mem[device_id],size2*sizeof(LST_LORs_scatter),cudaMemcpyDefault);
    	

		FILE* file_fp_y = fopen("fp_y.bin", "ab"); // Open the file in binary write mode
    	if (file_fp_y == NULL) {
        	printf("Error opening the file.");
        	return ;
    	}    	
		fwrite(fpvalue1_hosty, sizeof(float), size1, file_fp_y); // Write the binary data
		fclose(file_fp_y);
    	

		FILE* file_fp_x = fopen("fp_x.bin", "ab"); // Open the file in binary write mode
    	if (file_fp_x == NULL) {
        	printf("Error opening the file.");
        	return ;
    	}    	
		fwrite(fpvalue1_hostx, sizeof(float), size2, file_fp_x); // Write the binary data
		fclose(file_fp_x);

		/*
		FILE* file_sc_x = fopen("sc_x.bin", "wb"); // Open the file in binary write mode
    	if (file_sc_x == NULL) {
        	printf("Error opening the file.");
        	return ;
    	}   

		FILE* file_sc_y = fopen("sc_y.bin", "wb"); // Open the file in binary write mode
    	if (file_sc_y == NULL) {
        	printf("Error opening the file.");
        	return ;
    	}   
		*/
		...
		for (int i = 0; i < size1; i++){
			...
			//fwrite(_event_y[i].sc_coeff, sizeof(float),1,file_sc_y);
		}
		for (int j = 0; j < size2; j++){
			...
			//fwrite(_event_x[j].sc_coeff, sizeof(float),1,file_sc_x);
		}
		//fclose(file_sc_x);
		//fclose(file_sc_y);
...
void
cuda_em_recon::ComputeScatterFraction(PET_geometry& geometry, PET_data& data, ImageArray<float>& Emission_Image, ImageArray<float>& Attenuation_Image, string datpath, string emission_data_filename){
	// down sample emission and attenuation image
	ImageArray<float> downSampledEmissionImg(Emission_Image, 
										geometry.X_SAMP_DOWN_SAMPLED_FOR_SSS,
										geometry.Y_SAMP_DOWN_SAMPLED_FOR_SSS,
										geometry.Z_SAMP_DOWN_SAMPLED_FOR_SSS);
	ImageArray<float> downSampledAttImg(Attenuation_Image, 
										geometry.X_SAMP_DOWN_SAMPLED_FOR_SSS,
										geometry.Y_SAMP_DOWN_SAMPLED_FOR_SSS,
										geometry.Z_SAMP_DOWN_SAMPLED_FOR_SSS);
	Single_Scatter_Simulation SSS;
	SSS.setDataPath(datpath);
	SSS.setEmissionAndAttImg(downSampledEmissionImg,downSampledAttImg);
	SSS.setPromptData(data.PET_LST_event_list);	
	SSS.setProtocol(data._protocol);
	SSS.setImgPosition(data.getImageBedPosition());
	SSS.setOutputFilename("scatter_chunkSize_"+to_string(int(geometry.chunkSizeForSSS))+emission_data_filename);
	SSS.setChunkSize(geometry.chunkSizeForSSS);
	SSS.runSimulation();
	// divide PET_data into multiple chunks
	int startIndex = 0;
    int endIndex = 0;	
	vector<size_t> indexGroup; //ending index of each group, start with the end index of 1st group

    /*

	float currentMax = data.getInitialBedPosition();
    while (startIndex < data.PET_LST_event_list.size()) {
		currentMax += groupSize;
        auto it = upper_bound(events.begin() + startIndex, events.end(), currentMax,
                                   [](const float& val, const PET_LST_event& event){
                                       return val < event.bed_position;
                                   });

        size_t endIndex = it - events.begin() - 1;
		indexGroup.push_back(endIndex);
		cout << "Group (" << currentMax - groupSize << "mm to " << currentMax << "mm): Start Index = "
                  << startIndex << ", End Index = " << endIndex << endl;
        startIndex = endIndex + 1;
	}
	cout <<"chunk size:"<< groupSize  <<"mm, number of chunks:" << indexGroup.size()<<endl;
	//for each chunk, callculate the scatterFraction
	for(int i=0;i<1;i++){
		startIndex = i==0? 0:indexGroup[i-1]+1;
		endIndex = indexGroup[i];
		SSS.setSimulationParameters();
		SSS.runSimulation();
	}	
	*/
